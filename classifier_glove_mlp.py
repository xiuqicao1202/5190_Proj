"""
GloVe + MLP (Report 2.3.1)
Word sequence embedding {e_i}, average pooling h0 = (1/T)Σ e_i, MLP: h1 = ReLU(W1 h0 + b1), ˆy = σ(W2 h1 + b2).
"""
...
def _print_cuda_install_hints() -> None:
    print("[hint] WSL2: Install the latest NVIDIA driver for WSL on Windows; run `nvidia-smi` in your distribution to see your GPU.")
    print("[hint] PyTorch should be installed with the **CUDA version** wheel (CPU version will always result in cuda_available=False). Example:")
    print('       pip3 install torch --index-url https://download.pytorch.org/whl/cu124')
    print("       (Please check version at https://pytorch.org/get-started/locally/ )")


def _print_cuda_diagnostics() -> None:
    print(f"[diag] torch={torch.__version__!r}  torch.version.cuda={torch.version.cuda!r}")
    if torch.version.cuda is None:
        print("[diag] The current PyTorch is probably **CPU build**; you need to install the CUDA build.")
...
def resolve_torch_device(mode: str) -> torch.device:
    ...
    print("[device] auto → CPU (No CUDA available detected)")
    _print_cuda_diagnostics()
    return dev
...
def main() -> None:
    ...
    ap.add_argument("--glove", type=Path, default=None, help="GloVe file path; Default: Pretrained_Params/glove/glove.6B.100d.txt")
    ap.add_argument(
        "--skip-glove-download",
        action="store_true",
        help="Disable auto-download: if glove.6B.100d.txt is missing, exit directly (by default, glove.6B.zip will be auto-downloaded)",
    ...
    ap.add_argument(
        ...
        type=Path,
        default=None,
        help="Should contain Pretrained_Params/ and Finetune_Params/; Defaults to the directory of this script.",
    )
    ...
    write_training_txt(
        ...
        {
            "timestamp_and_notes": f"prefix={prefix}",
            "command_line": " ".join(sys.argv),
            "pretrained_embeddings": {"path": str(glove_txt), "default_location": str(default_glove_txt(proj))},
            ...
            "saved_files": {"checkpoint_pt": str(ck_path)},
        },
    )
    copy_script_source(meta_py, run_dir / f"{prefix}.py")
    print(f"[save] Finetune_Params/{prefix}/ written {prefix}_checkpoint.pt, {prefix}.txt, and {prefix}.py")

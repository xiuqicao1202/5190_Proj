...
def prepare_data(path: str) -> Tuple[List[str], List[str]]:
    """
    Convert URLs into pseudo-headline text and source labels.

    This keeps the same workflow as the TF-IDF baseline: X is a list of
    strings passed to model.predict(...), and y is the aligned label list.
    """
    df = pd.read_csv(path)
    urls = df["url"]

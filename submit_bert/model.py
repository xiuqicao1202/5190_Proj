"""
BERT submission model.
Loads fine-tuned weights and tokenizer from params_submit/bert_model.pt and provides predict().
"""
import os
import tempfile
...
_MODEL_PT = Path(__file__).resolve().parent / "bert_model.pt"
    ...
    def __init__(self) -> None:
        ...
        if not _MODEL_PT.exists():
            raise FileNotFoundError(
                f"bert_model.pt not found, please run classifier_bert.py to generate parameters: {_MODEL_PT}"
            )
        ...
        self._bert = AutoModelForSequenceClassification.from_config(config)
        self._bert.load_state_dict(ckpt["state_dict"])
        self._bert.eval()
        self._bert.to(self._device)  # Move model to CUDA if available
